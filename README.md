...
[vagrant@client ~]$ 
```
Проверим логи SELinux, чтобы понять в чём может быть проблема.
```
[root@client vagrant]# cat /var/log/audit/audit.log | audit2why 
[root@client vagrant]# 
```
Проверим логи SELinux сервере ns01
```
[root@ns01 vagrant]# cat /var/log/audit/audit.log | audit2why
type=AVC msg=audit(1715443377.012:1920): avc:  denied  { create } for  pid=5075 comm="isc-worker0000" name="named.ddns.lab.view1.jnl" scontext=system_u:system_r:named_t:s0 tcontext=system_u:object_r:etc_t:s0 tclass=file permissive=0
...
[root@ns01 vagrant]#
```
У приложения нет доступа к каталогу с записями dns. В конфигурации прописан католог /etc/named/*. Проверим контекст безопасности каталога.
```
...
```
Посмотреть в каком каталоги должны лежать, файлы, чтобы на них распространялись правильные политики SELinux можно с помощью
```
[root@ns01 vagrant]# sudo semanage fcontext -l | grep named
/etc/rndc.*                                        regular file       system_u:object_r:named_conf_t:s0 
/var/named(/.*)?                                   all files          system_u:object_r:named_zone_t:s0 
```
Проблема заключается в том, что конфигурационные файлы лежат в другом каталоге. Контекст безопасности неправильный.
...
;; WHEN: Sat May 11 16:33:08 UTC 2024
;; MSG SIZE  rcvd: 96
...
```
[root@client vagrant]# shutdown -r now
...
;; global options: +cmd
;; Got answer:
;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 7444
;; flags: qr aa rd ra; QUERY: 1, ANSWER: 1, AUTHORITY: 1, ADDITIONAL: 2
...
;; Query time: 2 msec
;; SERVER: 192.168.50.10#53(192.168.50.10)
;; WHEN: Sat May 11 16:35:50 UTC 2024
;; MSG SIZE  rcvd: 96
